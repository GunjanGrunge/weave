...
describe("buildConsultMuseResponse", () => {
  beforeEach(() => {
    ...
    generateSceneMock.mockResolvedValue({ text: "What if the car makes Eric complicit?", provider: "openai", model: "gpt-test" });
  });

  it("persists an editorial Muse turn without generating manuscript prose", async () => {
    await expect(buildConsultMuseResponse(
      "Bearer valid", { bookId: "book-1", message: "Make Eric feel guilty." }, keys,
    )).resolves.toEqual({
      statusCode: 200,
      body: { text: "What if the car makes Eric complicit?", provider: "openai", model: "gpt-test" },
    });
    expect(generateSceneMock).toHaveBeenCalledWith(
      "book-1", expect.stringContaining("Do not draft manuscript prose"), keys, "museConversation",
    );
    expect(appendMuseConversationMock).toHaveBeenCalledWith(
      "book-1", "Make Eric feel guilty.", "What if the car makes Eric complicit?",
    );
  });

  it("does not call the model for invalid input or unauthenticated requests", async () => {
    await expect(buildConsultMuseResponse("Bearer valid", { bookId: "book-1", message: "" }, keys)).resolves.toMatchObject({ statusCode: 400 });
    verifyIdTokenMock.mockRejectedValue(new AuthError("Missing token"));
    await expect(buildConsultMuseResponse("", { bookId: "book-1", message: "Hello" }, keys)).resolves.toMatchObject({ statusCode: 401 });
