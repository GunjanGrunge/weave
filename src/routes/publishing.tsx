...
export const Route = createFileRoute("/publishing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Formatting, metadata, ISBN, and launch planning." },
...
function PublishingPage() {
  ...
  const pct = publishingChecklist.length > 0 ? done / publishingChecklist.length : 0;
    ...
    <div className="mx-auto max-w-5xl animate-reveal px-6 py-10 lg:px-10">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[1fr_260px]">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs">
                {done} / {publishingChecklist.length}
              </span>
            </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="mt-2 font-display text-3xl italic">Not scheduled</div>
          <div className="mt-4 text-xs text-muted-foreground">No launch details yet.</div>
        </div>
      </div>
      ...
      <div className="mt-8 rounded-2xl border border-border bg-card">
        ...
        <ul className="divide-y divide-border">
          {publishingChecklist.map((c) => (
            <li key={c.id} className="flex items-start gap-4 p-5">
              ...
              <div className="flex-1">
                <div
                  className={`font-serif text-base ${c.done ? "text-muted-foreground line-through" : ""}`}
                >
                  {c.label}
                </div>
          ...
          {publishingChecklist.length === 0 && (
            <li className="p-5 text-sm text-muted-foreground">No checklist items yet.</li>
          )}
        </ul>
      </div>
    </div>
